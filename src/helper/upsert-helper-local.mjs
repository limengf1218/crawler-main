...
const cookie = 'mikomiko-token=' + SECURE_MIKOMIKO_TOKEN;
//-------------------------------------------------------------end----------------------------------------------------------------------------//
...
const minioClient = new Minio.Client({
    ...
    port: 9000,
    useSSL: false,
    accessKey: S3_UPLOAD_KEY,
    secretKey: S3_UPLOAD_SECRET
  });
...
async function deleteImage(bucket, key) {
    try {
        ...
        await minioClient.send(new DeleteObjectCommand(input));
...
export async function uploadMultipartFile(bucket, key, fileId, fileUrl, fileName, modelId, FILE_INDEX) {
    try {
        ...
        const createResponse = await minioClient.send(createCommand);
    ...
    async function uploadPart(
        ...
        try {
            ...
            const uploadPartResponse = await minioClient.send(uploadPartCommand);
    ...
    async function completeMultipartUpload(
        ...
        try {
            ...
            await minioClient.send(completeCommand);
        } catch (error) {
...
async function seedModelFile(file, modelVersionId, modelId, userId, FILE_INDEX) {
    ...
    await instance.post('trpc/modelFile.createSeed', {
        "json": {
            "name": file.name,
            "id": file.id,
            "sizeKB": file.sizeKB,
...
export async function seedPost(post, fileIndex) {
    try {
        ...
        // check if modelVersionId exists
        try {
            await instance.get(`trpc/modelVersion.getById?input={"json":{"id":${modelVersionId}}}`);
        } catch {
            await instance.post('trpc/modelVersion.upsertSeed', {
                "json": {
                    "modelId": -1,
